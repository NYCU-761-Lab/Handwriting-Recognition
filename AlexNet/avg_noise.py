...
"""*"""
a = torch.load('./avg_noise_params.pth')
net.load_state_dict(torch.load('./avg_noise_params.pth'))
"""*"""
...
def train():
    ...
    for epoch in range(EPOCH):
        ...
        # 保存模型參數
        torch.save(net.state_dict(), './avg_noise_params.pth')
...
def gaussian_noise(img, mean=0, std=0.3):
    ...
    noise = numpy.random.normal(mean, std, img.shape)
    # noise + 原圖
    ...
    noise = numpy.uint8(noise*255)

    # plt.imshow(gaussian_out, cmap='gray', interpolation='none')
    # plt.show()
...
if __name__ == "__main__":
    ...
    for img in imgData:
        ...
        imgData_gua.append(img)
        # plt.imshow(img, cmap='gray', interpolation='none')
        # plt.show()

    # for img in imgData_gua:
    #     plt.imshow(img, cmap='gray', interpolation='none')
    #     plt.show()
        
    results = predict(imgData_gua)          # 3. determine the answer for single image
    # print(results)
